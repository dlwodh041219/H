...
function runFaceTracking() {
  ...
  if (openFrameCount === requiredFrames) {
    detectedText = "4단계(간보기) 완료! 🎉 전체 미션 클리어!";
    stage = 4;
  }
}
...
function handleReps(isDoingGesture) {
  if (isDoingGesture) {
    if (!gestureActive) {
      ...
      if (stage === 0) detectedText = `1단계 재료 썰기: ${repCount}/${TARGET_REPS}`;
      if (stage === 1) detectedText = `2단계 팬 넣기: ${repCount}/${TARGET_REPS}`;
      if (stage === 2) detectedText = `3단계 볶기: ${repCount}/${TARGET_REPS}`;
...
function advanceStage() {
  ...
  if (prev === 0) detectedText = "1단계 완료! → 2단계로";
  if (prev === 1) detectedText = "2단계 완료! → 3단계로";
  if (prev === 2) detectedText = "3단계 완료! → 4단계(간보기)로";
  if (prev ===3) detectedText = "모든 단계 완료! 사랑하는 사람들과 음식을 나눠보세요🤤"
}
...
function drawStageInfo() {
  ...
  let txt = "";
  if (stage === 0) txt = "1단계) 재료 썰기: 오른손 위↔아래 3회!";
  else if (stage === 1) txt = "2단계) 팬 넣기: 양손 위↔아래 3회!";
  else if (stage === 2) txt = "3단계) 볶기: 양손 좌↔우 3회!";
  else if (stage === 3) txt = "4단계) 간보기: 입 벌리기!";
  else if (stage === 4) txt = "모든 단계 완료!"
...
function updateHistory(rw, lw) {
  ...
  if (rightWristYHist.length > HISTORY) { // 히스토리 길이가 길어지면 삭제
